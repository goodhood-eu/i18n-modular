...
const { debug, getOptions: getSharedOptions, getDeepSortedObject, getModuleId } = require('../utils');
const sharedSchema = require('../schema');

const sharedOptions = pick(sharedSchema.properties, 'keysRoot', 'moduleEnding');
...
const schema = {
  ...
  required: Object.keys(sharedOptions),
...
const loader = function(source) {
  const options = pick(getSharedOptions(getOptions(this)), ...Object.keys(schema.properties));
  validate(schema, options, { name: 'I18nModular module loader' });

  const { store, getId } = options;
  const defaultGetId = (filePath) => getModuleId(filePath, options.keysRoot, options.moduleEnding);
  const generateId = getId || defaultGetId;
