...
describe('utils', () => {
  it('rebase', () => {
    ...
    assert.equal(rebase('/abc'), '/abc', 'only context');
  ...
  it('getTranslations', () => {
    ...
    assert.isUndefined(getTranslations(), 'empty call');
    assert.isUndefined(getTranslations(webpackModule), 'empty module');

    setTranslations(webpackModule, content);
    assert.equal(getTranslations(webpackModule), content, 'extracted content properly');
  });

  it('setTranslations', () => {
    const content = 'something anything really';
    const webpackModule = {};
    assert.isObject(setTranslations(), 'empty call');
    assert.isObject(setTranslations({}), 'empty module');

    setTranslations(webpackModule, content);
    assert.equal(getTranslations(webpackModule), content, 'set content properly');
  });
});
