...
class I18nModularPlugin {
  ...
  apply(compiler) {
    const { moduleEnding, dictionaryPattern, emitFile } = this.options;
    const keysRoot = rebase(compiler.context, this.options.keysRoot);
      ...
      compilation.hooks.finishModules.tapPromise(PLUGIN_NAME, async(modules) => {
        ...
        const promises = modules.reduce((acc, webpackModule) => {
          if (dictionaryRegex.test(webpackModule.resource)) {
            ...
            debug('added %d language modules to %s, triggering rebuild', builtModules.length, webpackModule.resource);
