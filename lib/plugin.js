...
const { debug, getOptions, getDictionaryRegex, getLanguage } = require('./utils');
...
class I18nModularPlugin {
  ...
  apply(compiler) {
    ...
    const dictionaryRegex = getDictionaryRegex(path.join(compiler.context, dictionaryPattern));
