...
class I18nModularPlugin {
  ...
  apply(compiler) {
    ...
    const dictionaryRegex = getDictionaryRegex(outputPattern.replace(compiler.context, ''));
    ...
    compiler.hooks.emit.tapAsync(PLUGIN_NAME, (compilation, callback) => {
      ...
      Array.from(this.seeds.keys()).forEach((id) => {
        ...
        // Webpack is unable to save asseds to absolute paths, a relative to the output folder
        // must be provided. 🤷‍♂️
