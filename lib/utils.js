...
const RC_NAME = '.i18n-modular-rc';
const CONTEXT_NAME = 'I18N_MODULAR_CONTEXT';
const ID_PREFIX = 'module:';
const TRANSLATIONS_KEY = '__i18n_modular_translations';
...
const getContext = () => rebase(process.cwd(), process.env[CONTEXT_NAME]);

const getOptions = (options) => {
  let rc = {};
  try { rc = require(`${getContext()}/${RC_NAME}`); } catch (_) { /* no action required */ }
...
const storeTranslationModules = (webpackModule = {}, data) => {
  webpackModule[TRANSLATIONS_KEY] = data;
  return webpackModule;
};
const getTranslationModules = (webpackModule = {}) => webpackModule[TRANSLATIONS_KEY];

const collectModules = (map, language) => (
  Array.from(map.keys()).reduce((acc, key) => {
    acc[key] = map.get(key)[language];
    return acc;
  }, {})
);
